...
<body><body style="background-image: url(images/train1.jpg);background-repeat: no-repeat;background-size: cover;background-attachment: fixed;"></body>
  <?php 
    session_start();
    ...
    $query1=mysqli_query($con,"SELECT * FROM user_details WHERE user_details.mobile_number=BINARY $mobile ") or die(mysql_error());
    $query = mysqli_query($con,"SELECT * FROM user_details WHERE user_details.mobile_number=BINARY $mobile AND user_details.password=BINARY '".$pwd."' ") or die(mysql_error());
    ...
    if($row = mysqli_fetch_array($query))
    {
      ...
      $temp4=$row['lname'];
      $_SESSION["id"]=$temp2;
      //$query2 = mysqli_query($con," SELECT * from user_details,reservation where user_details.user_id=reservation.user_id AND  user_details.mobile_number=BINARY $mobile AND reservation.ticket_status='BOOKED' and reservation.journey_date>=CURDATE() ORDER BY reservation.journey_date") or die(mysql_error());

    	$query2 = mysqli_query($con," SELECT * from user_details,reservation where user_details.user_id=reservation.user_id AND  user_details.mobile_number=BINARY $mobile AND reservation.ticket_status='BOOKED' ORDER BY reservation.journey_date") or die(mysql_error());
